...
# ---------------------------------------------------------------------------
# Local schema equivalents (no api.* imports)
# ---------------------------------------------------------------------------
...
class FuzzyService:
    ...
    def build_candidate_query(self, normalized: NormalizedQuery) -> CandidateQueryPlan:
        # ── Exact match path ───────────────────────────────────────────
        if normalized.order_id and _ORDER_ID_RE.match(normalized.order_id):
            sql = """
                SELECT
        ...
        params: Dict[str, Any] = {"max_candidates": self.max_candidates}
        ...
        return sorted(
            (
                CandidateSummary(
                    ...
                    purchase_order_id=row.get("PURCHASE_ORDER_ID") or "",
                    status=row["STATUS"],
